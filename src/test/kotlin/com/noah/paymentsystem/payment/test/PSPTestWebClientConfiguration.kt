...
class PSPTestWebClientConfiguration(
    ...
    fun createTestTossWebClient(
        vararg customHeaderKeyValue: Pair<String, String>
    ): WebClient {
    ...
    private fun reactorClientHttpConnector(): ClientHttpConnector {
        return ReactorClientHttpConnector(
            HttpClient.create(
                ConnectionProvider.builder("test-toss-payment").build()
            )
        )
    }
}
